...
@st.composite
def dict_strategy(draw):
    keys = draw(st.lists(st.text(min_size=1, max_size=10), max_size=5, unique=True))
    values = draw(st.lists(st.one_of(st.integers(), st.text(), st.floats(), st.booleans()), min_size=len(keys), max_size=len(keys)))
...
def test_collection_snapshot(records_to_add:int, metadata:dict):
    with tempfile.TemporaryDirectory() as temp_dir:
        chroma_dir = os.path.join(temp_dir,"chroma")
        os.makedirs(chroma_dir, exist_ok=True)
        client = chromadb.PersistentClient(path=chroma_dir)
        col = client.get_or_create_collection("test_collection",metadata=metadata if len(metadata) > 0 else None)
        ...
        collection_snapshot(chroma_dir, "test_collection", Path(temp_dir,"snapshot", "snapshot.sqlite3"))
        assert os.path.exists(Path(temp_dir,"snapshot", "snapshot.sqlite3"))
        with get_sqlite_snapshot_connection(Path(temp_dir,"snapshot", "snapshot.sqlite3").absolute().as_posix()) as conn:
            cursor = conn.cursor()
            ...
            assert cursor.fetchone()[0] == 4 if records_to_add < 1000 else 5 # 5 if pickle metadata is included e.g. if added records > threshold
            cursor.execute("SELECT count(*) FROM segments")
            assert cursor.fetchone()[0] == 2
            cursor.execute("SELECT count(*) FROM segment_metadata")
            assert cursor.fetchone()[0] == 0
            cursor.execute("SELECT count(*) FROM max_seq_id")
            assert cursor.fetchone()[0] == 1 if records_to_add < 1000 else 2 # 2 if pickle metadata is included e.g. if added records > threshold
