...
def test_check_disk_space():
    # Create test directories with mock data
    with patch('chroma_ops.utils.get_dir_size') as mock_dir_size, \
         patch('chroma_ops.utils.get_disk_free_space') as mock_free_space:
        
        # Test case 1: Plenty of space available
        mock_dir_size.return_value = 1000  # 1KB source
        mock_free_space.return_value = 10000  # 10KB free
        assert check_disk_space("/fake/source", "/fake/target") == True

        # Test case 2: Not enough space
        mock_dir_size.return_value = 1000  # 1KB source
        mock_free_space.return_value = 1000  # 1KB free (less than 1.1x required)
        assert check_disk_space("/fake/source", "/fake/target") == False

        # Test case 3: Exactly at the threshold
        mock_dir_size.return_value = 1000  # 1KB source
        mock_free_space.return_value = 1100  # 1.1KB free (exactly 1.1x)
        assert check_disk_space("/fake/source", "/fake/target") == True

def test_get_disk_free_space(tmp_path):
