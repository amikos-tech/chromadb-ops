...
def test_basic_commit(records_to_add: int):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        vector_segments = [
            client._server._manager.get_segment(
        ...
        for segment in vector_segments:
            ...
            if records_to_add % batch_size == 0:
                assert len(segment._index.get_ids_list()) == records_to_add, "Precheck failed"
            else:
                assert len(segment._index.get_ids_list()) != records_to_add, "Precheck failed"
        cursor = conn.cursor()
        ...
        assert count.fetchone()[0] == records_to_add
        for segment in vector_segments:
            assert len(segment._index.get_ids_list()) == records_to_add, "postcheck failed"
